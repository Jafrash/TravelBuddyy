...
export class AIService {
  ...
  async generateItinerary(request: ItineraryRequest): Promise<string> {
    console.log(`[AIService] Starting itinerary generation for user ${request.userId}`);
    ...
    try {
      ...
      try {
        console.log(`[AIService] Checking database for existing itinerary`);
        const dbItinerary = await this.getItineraryFromDb(request);
        if (dbItinerary?.content) {
          console.log(`[AIService] Found existing itinerary in database`);
          this.cache.set(cacheKey, dbItinerary.content);
          return dbItinerary.content;
        }
      } catch (dbError) {
        console.error('[AIService] Error fetching from database, will continue:', dbError);
      ...
      console.log(`[AIService] Generating new AI itinerary for user ${request.userId}`);
      const prompt = await this.buildPrompt(request);
      console.log('[AIService] Generated prompt:', prompt.substring(0, 200) + '...');
      
      console.log('[AIService] Calling AI model...');
      ...
      console.log('[AIService] Successfully generated itinerary content');
      ...
      this.saveItineraryToDb({
        ...
        console.error('[AIService] Failed to save itinerary to DB (non-critical):', dbError);
      });
      
      return content;
    } catch (error) {
      console.error('[AIService] Error in generateItinerary:', error);
      if (error instanceof Error) {
        console.error('[AIService] Error stack:', error.stack);
      }
      
      // Return mock response on error
      console.log('[AIService] Falling back to mock response due to error');
      return this.generateMockItinerary(request);
    }
  }
